...
#Access to 
RUN  export PATH="~/.cabal/bin/:$PATH" && git clone https://github.com/input-output-hk/cardano-node.git /cardano && cd /cardano && git fetch --all --recurse-submodules --tags && git checkout tags/1.25.1 && cabal configure --with-compiler=ghc-8.10.4 && cd /cardano && echo "package cardano-crypto-praos" >>  cabal.project.local && echo "  flags: -external-libsodium-vrf" >>  cabal.project.local && cabal build --jobs=2 all
