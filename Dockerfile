...
#B RUN /bin/echo -ne "deb http://deb.debian.org/debian/ experimental main\ndeb-src http://deb.debian.org/debian/ experimental main" > /etc/apt/sources.list.d/experimental.list && /bin/echo -ne "Package: *\nPin: release a=experimental\nPin-Priority: 1\n" > /etc/apt/preferences.d/experimental.pref
#B RUN /bin/echo -ne "deb http://deb.debian.org/debian/ unstable main\ndeb-src http://deb.debian.org/debian/ unstable main" > /etc/apt/sources.list.d/unstable.list && /bin/echo -ne "Package: *\nPin: release a=unstable\nPin-Priority: 50\n" > /etc/apt/preferences.d/unstable.pref
...
RUN apt-get -y build-dep ghc \
  ...
  && make ${JOBS} \
  && make ${JOBS} install
  #&&  ~/.cabal/bin/cabal user-config update \
  #&& ~/.cabal/bin/cabal v2-install ${JOBS} alex happy --constraint="lukko -ofd-locking" \
  #&& PATH="~/.cabal/bin/:$PATH" hadrian/build.sh ${JOBS} binary-dist
  #&& ALEX=~/.cabal/bin/alex HAPPY=~/.cabal/bin/happy ./configure \
  # hadrian/build.sh ${JOBS} binary-dist

#Install target ghc
#A COPY install-ghc /
#A RUN /install-ghc ${TARGETARCH} ${GHC_VERSION} && ghc-pkg recache
#B RUN apt-get install --download-only ghc/experimental && dpkg -x /var/cache/apt/archives/ghc_*.deb / 
#C HAVE_OFD_LOCKING
#C RUN apt-get -y build-dep ghc
#C RUN ~/.cabal/bin/cabal v2-install alex happy --constraint="lukko -ofd-locking"
#C RUN git clone --recurse-submodules --tags https://gitlab.haskell.org/ghc/ghc.git /ghc 
...
RUN apt-get -y update && apt-get -y upgrade && apt-get -y install --no-install-recommends bash curl jq miniupnpc
