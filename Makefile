...
CARDANO_VERSION := 1.26.1
ARCH:= $(shell docker version -f "{{.Server.Arch}}")
all:
	docker build --build-arg JOBS="-j2" --build-arg TARGETARCH=`docker version -f "{{.Server.Arch}}"` --build-arg CARDANO_VERSION=$(CARDANO_VERSION) -t $(DOCKER_TAG):$(ARCH)-$(CARDANO_VERSION)  .
